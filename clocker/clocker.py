#!/usr/bin/env python
import argparse
import collections
import datetime
import os
import sys


KEYS = ('datetime', 'event', 'project')
Record = collections.namedtuple('Record', KEYS)
ClockedInRecord = collections.namedtuple(
    'ClockedInRecord', ('start', 'duration', 'project'))

DATETIME_FMT = "%Y-%m-%d %H:%M:%S.%f"

def get_clocker_filename():
    """Get the name of the clocker file from environment variable CLOCKER_FILE.

    If the environment variable is not set, defaults to ``.clocker`` in the
    current directory.

    """
    path = os.getenv('CLOCKER_FILE', '.clocker')
    return path


def get_record(pos=-1):
    """Read a single record from the clocker file. By default, reads the last
    record.

    """
    try:
        with open(get_clocker_filename()) as clocker_file:
            last_line = clocker_file.readlines()[pos]
            values = [token.strip() for token in last_line.split(',')]
            record = Record(*values)
    except (FileNotFoundError, IndexError):
        record = None
    return record


def get_records(start=None, end=None):
    """Return a list of records. By default, returns all records."""
    records = []
    start = start or datetime.datetime(1, 1, 1, 0, 0)
    end = end or datetime.datetime.now()
    next_event = 'IN'
    for line in open(get_clocker_filename()):
        values = [token.strip() for token in line.split(',')]
        record = Record(
            datetime=datetime.datetime.strptime(values[0], DATETIME_FMT),
            event=values[1],
            project=values[2])
        if record.event == next_event:
            if record.event == 'IN':
                if start <= record.datetime and record.datetime <= end:
                    records.append(record)
                    next_event = 'OUT'
            else:
                # If the next event is a clock out, always record it. This
                # ensures that a work session started near the end of a week
                # (say at 20:00 on Saturday night) and ended at the beginning
                # of the following week (say at 02:00 on Sunday morning) is
                # completely accounted for.
                records.append(record)
                next_event = 'IN'
    return records


def is_clocked_in():
    """Read clocker file and determine if last entry is a clock in event.
    Return true if it is, false otherwise.

    """
    last_record = get_record()
    if last_record and 'IN' in last_record.event:
        return True
    return False


def clock_in(args):
    """Record a clock in event in the clocker file."""
    if is_clocked_in():
        print('aleady clocked in', file=sys.stderr)
        return
    project = args.project or 'default'
    print('clocking in project {}'.format(project))
    now = datetime.datetime.now()
    with open(get_clocker_filename(), 'a') as clocker_file:
        clocker_file.write('{},IN,{}\n'.format(now, project))


def clock_out(args):
    """Record a clock out event in the clocker file."""
    if not is_clocked_in():
        print('not clocked in', file=sys.stderr)
        return
    last_record = get_record()
    project = last_record.project
    print('clocking out of project {}'.format(project))
    now = datetime.datetime.now()
    with open(get_clocker_filename(), 'a') as clocker_file:
        clocker_file.write('{},OUT,{}\n'.format(now, project))


def get_week_boundaries(date=None):
    """Return datetimes for the beginning and end of the week containing the
    given date."""
    date = date or datetime.datetime.now().date()

    # Week starts on Sunday, get previous Sunday.
    start = date - datetime.timedelta(date.weekday() + 1)
    # Convert to datetime at 00:00:00.
    start = datetime.datetime(start.year, start.month, start.day)

    # Week ends on Saturday, get next Saturday.
    end = start + datetime.timedelta(6)
    # Convert to datetime at 23:59:59.999.
    end = datetime.datetime(end.year, end.month, end.day, 23, 59, 59, 999000)

    return start, end


def clocked_in_times(records):
    """Get timedeltas during which status was clocked in."""
    timedeltas = []
    records_iter = iter(records)
    for record in records_iter:
        if record.event == 'IN':
            end_record = next(records_iter)
            assert end_record.event == 'OUT'
            assert record.project == end_record.project
            in_record = ClockedInRecord(
                start=record.datetime,
                duration=end_record.datetime - record.datetime,
                project=record.project)
            timedeltas.append(in_record)
    return timedeltas


def report(start=None, end=None):
    """Print a report of clocked in time per project for a given time interval.

    By default, prints the report for the past week.

    """
    if not start and not end:
        date = datetime.datetime.now().date() - datetime.timedelta(7)
        start, end = get_week_boundaries(date)
    timedeltas = clocked_in_times(get_records(start=start, end=end))
    projects = set([delta.project for delta in timedeltas])

    column_names = ['project', 'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Total']
    rows = [column_names]
    for project in projects:
        row = [project] + [0 for i in range(8)]
        deltas = [delta for delta in timedeltas if delta.project == project]
        for delta in deltas:
            weekday = delta.start.weekday()
            row[(weekday + 1) % 7 + 1] += delta.duration.total_seconds() / 3600
        row[-1] = sum(row[1:-1])
        rows.append(row)
    return rows


def print_report(args):
    date = datetime.datetime.now().date() + 7 * datetime.timedelta(args.week)
    start, end = get_week_boundaries(date)
    print('Report for week from {} to {}'.format(start.date(), end.date()))
    rep = report(start, end)
    formats = ['{:<10s}'] + 8 * ['{:>10.2f}']
    header_formats = ['{:<10s}'] + 8 * ['{:>10s}']
    for header, fmt in zip(rep[0], header_formats):
        print(fmt.format(header), end='')
    print()

    for row in rep[1:]:
        for element, fmt in zip(row, formats):
            print(fmt.format(element), end='')
        print()


def main():
    parser = argparse.ArgumentParser(description='Simplistic time tracking')
    subparsers = parser.add_subparsers(required=True, dest='command')
    in_parser = subparsers.add_parser('in', help='Clock in the desired project')
    in_parser.add_argument('project', help='Project name', nargs='?', default=None)
    in_parser.set_defaults(func=clock_in)
    out_parser = subparsers.add_parser('out', help='Clock in the desired project')
    out_parser.set_defaults(func=clock_out)
    report_parser = subparsers.add_parser('report', help='Clock in the desired project')
    report_parser.add_argument('week',
        help='Week number relative to current week (default is -1, previous week)',
        nargs='?', default=-1, type=int)
    report_parser.set_defaults(func=print_report)

    args = parser.parse_args()
    args.func(args)


if __name__ == '__main__':
    main()

